        ...
        for cmd in cmds {
            match cmd {
                ...
                ControlCommand::FetchHistorical { req_id, con_id, symbol, exchange, sec_type, end_date_time, duration, bar_size, what_to_show, use_rth } => {
                    self.send_historical_request(req_id, con_id, &exchange, &sec_type, &end_date_time, &duration, &bar_size, &what_to_show, use_rth);
                    let _ = symbol;
                }
                ...
                ControlCommand::FetchContractDetails { req_id, con_id, symbol, sec_type, exchange, currency, maturity } => {
                    if con_id > 0 {
                        ...
                        self.send_secdef_request_by_symbol(req_id, &symbol, &sec_type, &exchange, &currency, &maturity);
                    }
                }
...
impl HotLoop {
    ...
    fn send_historical_request(&mut self, req_id: u32, con_id: i64, exchange: &str, sec_type: &str, end_date_time: &str, duration: &str, bar_size: &str, what_to_show: &str, use_rth: bool) {
    ...
    fn send_secdef_request_by_symbol(&mut self, req_id: u32, symbol: &str, sec_type: &str, exchange: &str, currency: &str, maturity: &str) {
        if let Some(conn) = self.ccp_conn.as_mut() {
            let req_id_str = req_id.to_string();
            let ts = chrono_free_timestamp();
            ...
            if !maturity.is_empty() {
                tags.push((200, maturity));
            }
            let _ = conn.send_fix(&tags);
...
mod tests {
    ...
    fn wire_historical_roundtrip_command_to_response() {
        ...
        tx.send(ControlCommand::FetchHistorical {
            ...
            exchange: String::new(), sec_type: String::new(),
